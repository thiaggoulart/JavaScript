...
export class PostsService {
  ...
  buscarTodosPosts(): Observable<Post[]> {
    ...
    .pipe(catchError(this.tratadorErros<Post[]>([])));
  }

  adicionarPost(umPost:Post): Observable<Post>{
    const opcoesHttp = {
      headers: new HttpHeaders({
        'Content-type': 'application/json'
      })
    };
    return this.http.post<Post>(this.urlBase, umPost, opcoesHttp)
    .pipe(catchError(this.tratadorErros<Post>()));
  5}

  private tratadorErros<T>(resultado?:T){
